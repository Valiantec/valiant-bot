...
class Command extends BaseCommand {
  ...
  async execute() {
    ...
    if (!text) {
      throw new UserError("You can't warn a member without providing a reason");
    }
    ...
    multiIDStringToList(memberIds).forEach(async memberId => {
      try {
        ...
        this.dMsg.channel.send(`✅ Warned : <@${memberId}>`).catch(err => console.log(err));
      } catch (e) {
        console.log(e);
        this.dMsg.channel.send(`❌ Failed to warn <@${memberId}>`).catch(err => console.log(err));
      }
    });
  }
}
